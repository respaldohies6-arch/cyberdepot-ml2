...
/* =========================================================
 * HOVER DEL TEXTO / ENLACES
 * Evita que Astra/WooCommerce lo convierta en blanco.
 * ========================================================= */
.cdpc-producto a,
.cdpc-producto a:visited,
.cdpc-producto a:focus,
.cdpc-producto a:hover,
.cdpc-producto-nombre,
.cdpc-producto-nombre a,
.cdpc-producto-nombre a:visited,
.cdpc-producto-nombre a:focus,
.cdpc-producto-nombre a:hover{
...
(function(){
    ...
    function compatibility(product){
        ...
        const others =
            Object.values(selected)
                ...
                .filter(function(other){
                    return other && other.component !== product.component;
                });
        ...
        /*
         * Regla general:
         * si no existe una regla de incompatibilidad conocida entre
         * dos componentes, NO los marcamos como "Revisar".
         * El configurador debe mostrar como compatibles las
         * combinaciones normales de PC y reservar amarillo para
         * cuando sí existe una relación comprobable pero faltan datos.
         */
        let result = 'green';
        ...
        others.forEach(function(other){
            ...
            const pair = [product.component, other.component]
                .sort()
                .join('|');
            ...
            if(pair === 'cpu|motherboard'){
                ...
                if(product.socket && other.socket){
                    if(product.socket !== other.socket){
                        incompatible();
                    }
                } else {
             ...
             * MOTHERBOARD ↔ RAM
             * ================================================== */
            if(pair === 'motherboard|ram'){

                if(product.ddr && other.ddr){
                    if(product.ddr !== other.ddr){
                        incompatible();
                    }
                } else {
            ...
            if(pair === 'motherboard|storage'){
                ...
                /* NVMe/M.2 requiere soporte M.2, NVMe o PCIe. */
                if(drive.nvme || drive.m2){
                    if(!(board.nvme || board.m2 || board.pcie)){
                        /* Si la ficha de la motherboard no trae
                         * información de almacenamiento, no asumimos
                         * incompatibilidad: sólo falta información. */
                        if(!(board.sata || board.pcie || board.m2 || board.nvme)){
                            warning();
                        } else {
                            incompatible();
                        }
                ...
                /* SATA requiere SATA. */
                if(drive.sata){
                    if(!(board.sata || board.m2 || board.pcie)){
                        if(!(board.nvme || board.sata || board.m2 || board.pcie)){
                            warning();
                        } else {
                            incompatible();
                        }
            ...
            if(pair === 'case|motherboard'){
                ...
                const bf = norm(board.formFactor);
                const gf = norm(gabinete.formFactor);
                ...
                if(bf && gf){
                    ...
                    if(caseIsATX){
                        if(!(boardIsATX || boardIsMATX || boardIsITX)){
                            incompatible();
                        }
                    }
                    else if(caseIsMATX){
                        if(!(boardIsMATX || boardIsITX)){
                            incompatible();
                        }
                    }
                    else if(caseIsITX){
            ...
            if(pair === 'gpu|psu'){
                ...
                if(gpu.watts && psu.watts){
                    if(psu.watts < gpu.watts){
                        incompatible();
                    }
                } else {
                    warning();
             ...
             * GPU ↔ MOTHERBOARD
             *
             * Las GPUs PCIe son compatibles de forma general con
             * motherboards modernas; no las marcamos como "Revisar"
             * sólo por no tener una ficha PCIe completa.
             * ================================================== */
             ...
             * GPU ↔ CASE
             *
             * Sin una longitud de GPU y espacio de gabinete fiable,
             * no se considera una incompatibilidad automática.
             * ================================================== */
            if(pair === 'case|gpu'){
                return;
            }
            ...
            /* ==================================================
             * TODOS LOS DEMÁS PARES
             *
             * No existe una contradicción conocida: compatible.
             * Esto incluye lectores de memoria.
             * ================================================== */
        });
    ...
    function applyProductState(el){
        ...
        /*
         * Re-sincroniza el checkbox con el estado real.
         * Esto evita que Lectores de Memoria (y cualquier otro
         * componente) aparezca deseleccionado después de refreshAll().
         */
        if(checkbox){
            const current = selected[data.component];
            checkbox.checked = !!(
                current &&
                String(current.id) === String(data.id)
            );
        }
